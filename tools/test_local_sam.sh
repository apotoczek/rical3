...
rm -rf .aws-sam/build
sam build --no-cached >"$SAM_LOG" 2>&1

log "Starting sam local start-api on port ${PORT}"
sam local start-api --port "$PORT" >>"$SAM_LOG" 2>&1 &
SAM_PID=$!

log "Waiting for API readiness"
START_TS=$(date +%s)

until curl -sf "${BASE_URL}${HEALTH_PATH}" >/dev/null 2>&1; do
  sleep 1
  NOW_TS=$(date +%s)
...
RESPONSE="$(curl -sf "${BASE_URL}${HEALTH_PATH}")"
log "Response: ${RESPONSE}"

if echo "$RESPONSE" | grep -q '"status"[[:space:]]*:[[:space:]]*"healthy"'; then
  log "Health check passed"
else
  fail "Unexpected health response: ${RESPONSE}"
fi
